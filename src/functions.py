...
def on_search_click(event, filters, worklist):
    ...
    if firstName != "":
        patientName = (lastName + "*" + "^" + firstName + "*").translate(
            str.maketrans(
                "ěščřžýáíéůúňďťĚŠČŘŽÝÁÍÉŮÚŇĎŤ", "escrzyaieuundtESCRZYAIEUUNDT"
            )
        )
    else:
        patientName = (lastName + "*").translate(
            str.maketrans(
                "ěščřžýáíéůúňďťĚŠČŘŽÝÁÍÉŮÚŇĎŤ", "escrzyaieuundtESCRZYAIEUUNDT"
            )
        )
    ...
    logging.debug(
        f"Filter data: Date: {dateFrom, dateTo}, Patient ID: {patientId}, Last Name: {lastName}, First Name: {firstName}, Accession Number: {accessionNumber}"
    )
    ...
    dicom_data = []
    assoc = ae.associate(
        config["DICOM_SETTINGS"]["mwlScpIpAddress"],
        int(config["DICOM_SETTINGS"]["mwlScpPort"]),
        None,
        config["DICOM_SETTINGS"]["mwlScpAet"],
    )
    ...
    if assoc.is_established:
        ...
        message = wx.MessageDialog(
            None,
            load_text("infoMwlFailed"),
            load_text("errorDialog"),
            wx.OK | wx.ICON_ERROR,
        )
        message.ShowModal()
        message.Destroy()
    ...
    for i in range(len(dicom_data) - 1):
        ...
        if studyInstanceUid not in dictFoundsStudies.keys():
            dictFoundsStudies[studyInstanceUid] = [
                patientName,
                patientId,
                birthDate,
                accessionNumber,
    ...
    for studyInstanceUid, values in dictFoundsStudies.items():
        ...
        worklist.dvcWorklist.AppendItem(
            [
                studyDate,
                str(patientName),
                str(patientId),
...
def on_dvcWorklist_select(event, worklist, entered):
    ...
    entered.stPatientNameEntered.SetLabel(
        worklist.dvcWorklist.GetTextValue(
            selectedIndex,
            get_column_index_by_name(worklist, load_text("colPatientName")),
        )
    )
    entered.stPatientIdEntered.SetLabel(
        worklist.dvcWorklist.GetTextValue(
            selectedIndex, get_column_index_by_name(worklist, load_text("colPatientId"))
        )
    )
    entered.stAccessionNumberEntered.SetLabel(
        worklist.dvcWorklist.GetTextValue(
            selectedIndex,
            get_column_index_by_name(worklist, load_text("colAccessionNumber")),
        )
    )
    entered.stStudyUidEntered.SetLabel(
        worklist.dvcWorklist.GetTextValue(
            selectedIndex, get_column_index_by_name(worklist, load_text("colStudyUid"))
        )
    )
...
def on_save_input_data_click(event, dialog, entered):
    ...
    if patientId != "" and firstName != "" and lastName != "":
        ...
        dialog = wx.MessageDialog(
            dialog, message, load_text("warningDialog"), wx.OK | wx.ICON_WARNING
        )
        dialog.ShowModal()
        dialog.Destroy()
...
def isValidImage(path):
    ...
    if image.IsOk():
        ...
        logging.warning(f"Invalid or Corrupted Image in Hot Folder! '{path}")
        return False
    ...
    if os.path.isdir(directory):
        ...
        for f in os.listdir(directory):
            ...
            if (
                ...
                # and os.path.splitext(f)[1].lower() in validExtensions
                and isValidImage(fullPath)
            ):
                validFiles.append(f)
            else:
                logging.warning(f"File skipped: {f}")

        if validFiles:
            files2send.lbLoadedFiles.Set(validFiles)  # Nastavíme soubory do ListBoxu
...
def preview_selected_image(event, files, preview):
    ...
    if bitmap:
        ...
        preview.stError.SetLabel("")
    else:
        preview.imDisplay.SetBitmap(wx.NullBitmap)
        preview.stError.SetLabel(load_text("unsupportedFile"))
...
def convert_to_dicom(
    ...
    ds.SOPInstanceUID = f"{seriesUid}.{fileNumber}"
    ...
    if config["DICOM_SETTINGS"]["institutionName"]:
        ds.InstitutionName = config["DICOM_SETTINGS"]["institutionName"]
    if config["DICOM_SETTINGS"]["institutionAddress"]:
        ds.InstitutionAddress = config["DICOM_SETTINGS"]["institutionAddress"]

    # Convert image to JPEG and encapsulate
    output = io.BytesIO()
...
def send_dicom_file(ds, file):
    ...
    ae.add_requested_context(
        SecondaryCaptureImageStorage, transfer_syntax=JPEGExtended12Bit
    )

    logger.debug(f"Attempting to send DICOM file {file}")

    assoc = ae.associate(
        config["DICOM_SETTINGS"]["ipAddress"],
        int(config["DICOM_SETTINGS"]["port"]),
        None,
        config["DICOM_SETTINGS"]["aec"],
    )
    if assoc.is_established:
        try:
            dataset = ds
...
def send_to_aec(event, entered, files, preview):
    ...
    if patientName == "" and patientId == "":
        entered = wx.MessageDialog(
            entered,
            load_text("warningNoInfoStudy"),
            load_text("warningDialog"),
    ...
    if files.lbLoadedFiles.GetString(0) == load_text("txNoFilesLoaded"):
        files = wx.MessageDialog(
            entered,
            load_text("warningNoImageInDir"),
            load_text("warningDialog"),
            wx.OK | wx.ICON_WARNING,
        )
        files.ShowModal()
    ...
    if preview.stError.GetLabel() != "":
        files = wx.MessageDialog(
            entered,
            load_text("warningBadImageInDir"),
            load_text("warningDialog"),
            wx.OK | wx.ICON_WARNING,
        )
        files.ShowModal()
    ...
    logging.info(
        f"Study info: patient name: {patientName}, patient id: {patientId}, acc no: {accessionNumber}, date: {studyDate},time: {studyTime}, uid: {studyUid } "
    )

    file_count = len(files.lbLoadedFiles.GetItems())
    dlg = wx.ProgressDialog(
        load_text("infoFileProcessing"),
        load_text("infoFileSendings"),
        file_count,
        entered,
    ...
    for image in files.lbLoadedFiles.GetItems():
        ...
        if dicomFile is None:
            logging.error(
                "dicomFile is None. Error in function convert_to_dicom. Wrong image type?"
            )
            statusSend = False
            break
        ...
        if statusSend:
            ...
            statusSend = False
            logging.error("statusSend is False. Error in function convert_to_dicom.")
            break
...
def after_send_dicom_study_proccess(entered, preview, statusSend):
    ...
    if statusSend:
        message = wx.MessageDialog(
            entered,
            load_text("infoSuccessfullySent"),
            load_text("infoDialog"),
...
def after_send_dicom_file_proccess(ds, file):
    ...
    if (
        config["FILES_SETTINGS"].getboolean("moveImagesAfterSend") == True
        and config["FILES_SETTINGS"].getboolean("deleteImagesAfterSend") == False
    ):
...
def save_dicom_file(file, ds):
    ...
    try:
        ...
        destination = (
            destination + "\\" + os.path.basename(os.path.splitext(file)[0]) + ".dcm"
        )
    except:
        destination = (
            os.getcwd()
...
def on_click_static_text(event, entered):
    ...
    dialog = wx.MessageDialog(
        entered,
        load_text("infoAboutProgram"),
        load_text("infoAboutProgramTitle"),
        wx.OK,
    )
    dialog.ShowModal()
    dialog.Destroy()
